...
import com.algasensors.device.management.domain.exception.SensorDisabledException;
...
public class DisableSensorUseCaseImpl implements DisableSensorUseCase {
    ...
    public void execute(Command command) {
        ...
        if (Boolean.FALSE.equals(sensor.getEnabled())) {
            log.warn("Temperature is already disabled for sensor {}", sensor.getId());
            throw new SensorDisabledException(sensor.getId());
        }
        ...
        sensorClient.stopTransmission(sensorId);
    }
}
